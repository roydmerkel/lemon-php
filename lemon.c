    ...
    if( rp->rhsalias[i] && !used[i] ){
      ...
      if( has_destructor(rp->rhs[i],lemp) ){
        if (target_lang == LANG_C) {
          ...
          append_str("  $this->yy_destructor(yypParser,%d,$this->yystack[$this->yyidx + %d]->minor);\n", 0,
             rp->rhs[i]->index,i-rp->nrhs+1);
        }
      }else{
...
static void emit_define_with_prefixv(FILE *out, int *lineno, const char *prefix, const char *name,
  const char *fmt, va_list ap)
{
  if (target_lang == LANG_C) {
    fprintf(out, "#define %s%-30s", prefix, name);
    vfprintf(out, fmt, ap);
    fprintf(out, "\n");
    (*lineno)++;
  } else if (target_lang == LANG_PHP) {
    fprintf(out, "  const %s%s = ", prefix, name);
    vfprintf(out, fmt, ap);
    fprintf(out, ";\n");
    (*lineno)++;
  }
  ...
  const char *prefix, const char *name, const char *fmt, ...)
{
  ...
  va_start(ap, fmt);
  emit_define_with_prefixv(out, lineno, prefix, name, fmt, ap);
  va_end(ap);
}
...
void ReportTable(
  ...
  if (target_lang != LANG_PHP) {
    emit_define(out,&lineno,"YYCODETYPE","%s",minimum_size_type(0, lemp->nsymbol+1));
  }
  emit_define(out,&lineno,"YYNOCODE","%d",lemp->nsymbol+1);
  if (target_lang != LANG_PHP) {
    emit_define(out,&lineno,"YYACTIONTYPE","%s",minimum_size_type(0, lemp->nstate+lemp->nrule+5));
  }
  if( lemp->wildcard ){
    emit_define(out,&lineno,"YYWILDCARD","%d",lemp->wildcard->index);
  }
  ...
  */
  ...
  emit_define(out,&lineno,"YY_ACTTAB_COUNT","(%d)",n);
  if (target_lang == LANG_C) {
  ...
  emit_define(out,&lineno,"YY_SHIFT_COUNT","(%d)",n-1);
  emit_define(out,&lineno,"YY_SHIFT_MIN","(%d)",mnTknOfst);
  emit_define(out,&lineno,"YY_SHIFT_MAX","(%d)",mxTknOfst);
  if (target_lang == LANG_C) {
  ...
  emit_define(out, &lineno,"YY_REDUCE_COUNT","(%d)", n-1);
  emit_define(out, &lineno,"YY_REDUCE_MIN","(%d)", mnNtOfst);
  emit_define(out, &lineno,"YY_REDUCE_MAX","(%d)", mxNtOfst);
  if (target_lang == LANG_C) {
